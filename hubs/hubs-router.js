...
// by the time this runs, the /api/hubs part of the url
// has been evaluated.
// we only care about the rest of the url

// list of hubs
router.get("/", (req, res) => {
  // all the database methods return a promise

  const query = req.query; // holds the query string parameters

  Hubs.find(query) // pass the query string parameters to the database library
    .then((hubs) => {
